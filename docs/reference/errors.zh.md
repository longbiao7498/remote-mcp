...
Write 在正常操作中不产生 `"Error: ..."` 字符串——SFTP 失败（如权限拒绝）以未捕获的 `IOError` 异常传播，由服务器层的重试路径捕获后作为 `Error: <exception message>` 返回。
...
| `old_string` 出现多次且 `replace_all=False` | `Error: old_string found <N> times in <file_path>. Provide more context to match uniquely.` |
