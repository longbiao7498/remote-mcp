...
| `path` | string | 是 | — | 远程主机上的起始文件或目录。当为目录时递归搜索。远程绝对路径，或相对于已配置 cwd 的相对路径。不支持 ~（请使用绝对路径或相对路径）。 |
...
**成功时：** 按 `output_mode` 描述的原始 `grep` 输出，截断至 `head_limit` 行。输出路径为绝对路径（如 `/opt/app/foo.py`）——已将解析后的搜索根目录纳入路径中。如需相对路径输出，请在 Bash 调用中使用 `cd <cwd> && grep ...`。
