...
- **网络故障不会自动重试。** v0.2.2：若 SSH 在 Edit 执行过程中断开，agent 将收到 `Error: <ExceptionType>: <message>`，下一次工具调用会触发重连。Edit **不会**自动重试，因为读-改-写的语义使其不安全——重新执行会看到已被修改的文件，并错误地报告 `old_string not found`。Agent 应先通过 `Read` 读取文件，确认首次操作是否已成功。
