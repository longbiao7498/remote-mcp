...
**决策：** 彻底移除持久 bash 会话。每次 Bash 工具调用都打开一个新的 exec 通道，运行命令（用快照 `source` 包装），然后关闭通道。

**为何：** 三个原因共同促成了这一决定。第一，与 Claude Code 原生对齐：直接测试表明 CC 自身的 Bash 工具在每次调用之间都会重置 cwd 和环境变量——我们的持久模型是一个无意为之的偏差，对基于 CC 惯例训练的 agent 造成了令人困惑的行为。第二，一类系统性 bug：带 PTY 的持久会话会导致读取 stdin 的命令（`srun`、无参数的 `cat`、REPL 等）无限挂起，因为我们的持久 PTY 意味着远端命令永远看不到 EOF。第三，机制复杂性：持久模型需要 sentinel 协议、防止缓冲区死锁的后台 reader 线程、用于中断传递的 PTY 分配、后台命令的 `setsid` 包装以及精细的初始化序列——大约 350 行支撑机制代码，且有非平凡的失效模式。

非持久模型将这些缩减至约 50 行代码且无边界情况，同时通过快照机制保留了「环境只加载一次」的便利性：建立连接时，`bash -ic 'declare -p; declare -fp; alias'` 捕获 bashrc 环境并写入远程快照文件；每次 Bash 调用在运行前 `source` 此快照。

完整分析请参阅[为何采用非持久 Bash](./why-non-persistent-bash.md)。

## 可配置 cwd（v0.2.0）

**决策：** 允许在注册时为每台主机配置 `cwd`（`--cwd /opt/myapp` 或 `config.yaml` 中的 `cwd:` 字段）。所有工具以此 cwd 为锚点解析相对路径。所有工具输出统一在末尾追加 `[host=X cwd=Y]` 后缀。

**为何：** 在 v0.1.x 中，每个工具都需要绝对路径——`Read("config.yaml")` 会报错。Claude Code 原生的工作方式不同：它从 `claude` 启动时所在目录解析相对路径。这让 remote-mcp 像一个二等工具，并迫使 agent 在脑中维护绝对路径。可配置 cwd 弥合了这一差距：在 `/opt/myapp` 上工作的 agent 以 `--cwd /opt/myapp` 注册服务器，就能像在本地一样使用相同的相对路径惯例。

cwd 在注册时固定，而非由 agent 控制，原因有二：这镜像了 CC 原生的工作方式（会话 cwd 不会改变），也避免了在已经非持久的 Bash 模型之上再叠加一个并行有状态 `cd` 机制的混乱。

`[host=X cwd=Y]` 后缀加在所有输出上（不只是 Bash），以便 agent 无论刚调用了哪个工具，都始终有一个关于相对路径锚点的明确提示。

完整分析请参阅[可配置 cwd 与路径解析](./cwd-and-path-resolution.md)。
