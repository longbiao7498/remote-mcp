...
**v0.3.0 已实施。** A–K 阶段全部完成。权威设计为：
- `docs/superpowers/specs/2026-05-26-remote-mcp-design.md`（v2，基础）
- `docs/superpowers/specs/2026-05-27-v0.2.0-non-persistent-bash.md`（v0.2.0 增量）
- `docs/superpowers/specs/2026-05-28-network-robustness-design.md`（v0.2.2 增量）
- `docs/superpowers/specs/2026-05-31-v0.3.0-job-panel.md`（v0.3.0 增量——任务面板）

新会话：提议架构变更前**请通读相关规范**。许多决策都有明确的理由（snapshot 机制、cwd 策略、`~` 两层语义、统一后缀、SFTP-vs-exec 分离、本地优先面板元数据、sid 派生、终态不重复观察、JobArchive 纯本地操作），不应在未与用户确认的情况下重新讨论。

## 建设内容

`remote-mcp` 是一个**本地** Python MCP 服务器，向 Claude Code 公开 17 个工具。13 个通过 SSH 在远程 Linux 主机上运行；1 个（Feedback）写入本地 JSONL 文件；4 个（Jobs、JobKill、JobArchive、JobScript）使用本地优先元数据管理后台任务面板。七个与 Claude Code 原生工具有对应的（Read/Write/Edit/MultiEdit/Bash/Glob/Grep）与其 schema 和输出格式匹配。硬约束：远程主机仅有 SSH（无 agent 安装）、传输是 stdio MCP、SSH 库是 paramiko。

权威设计为仓库状态部分列出的四份规范。仓库根目录的中文版本 `软件设计文档.md` 是先前的 v1 草稿——保留供参考但**被 v2 取代**。

### 任务面板（v0.3.0）

v0.3.0 新增后台任务面板：通过 `Bash(run_in_background=True, name=..., log_path=...)` 启动的具名、可查询后台任务。

**新工具**：`Jobs`（列出/查询，含实时状态）、`JobKill`（单次打包 exec 发送 kill + 验证）、`JobArchive`（纯本地 mv 到 archive/ 或 zombie/）、`JobScript`（挂载自定义状态脚本）。

**本地优先存储**：面板元数据存储在 MCP 宿主机的 `~/.local/share/remote-mcp/jobpane/<sid>/<host>/` 目录中。远端仅保留扁平命名的文件：pid、status.sh 缓存及日志。这使面板查询成本极低（列表模式仅本地 IO；状态刷新至多一次批量 exec）。

**状态缓存工作流**：Jobs 与 JobKill 将观察到的状态写回本地 `<id>-meta.json`。终态（stopped、killed）不再重复观察——避免 PID 复用产生的误判。JobArchive 读取缓存状态，执行零远端操作。

**关键限制**：面板状态不跨 Claude Code 重启保留（新 CC 进程 → 新 PPID → 新 `sid`）。任务仍在远端运行。恢复方式：`Bash("ls ~/.cache/remote-mcp-*-pid")`。

设计理念：`docs/explanation/job-panel.md`（中文版：`job-panel.zh.md`）。
...
- Bash 带 `run_in_background=true` 接受 `name`（任务别名，在活跃面板任务中必须唯一）和 `log_path`（显式远端日志路径）。将命令包装在 `( setsid nohup bash --noprofile --norc -c 'source <snapshot>; ...' > <log_path> 2>&1 </dev/null & PID=$!; echo $PID > ~/.cache/remote-mcp-<sid>-<id>-pid; ... echo "BG_PID=$PID" ... )` 中。返回 `id / name / log_path / pid / started_at`。PID 同步确认——若 exec 响应丢失，回退到 SFTP 读取 pid 文件；两者均失败时任务**不**进入面板，返回 Error。`setsid` 是**非可选的**——使后台进程脱离 exec 通道的 session。
...
│                      #   + exec_with_snapshot(conn, command, timeout) -> ExecResult 辅助函数
├── paths.py           # resolve_path(path, cwd) helper，供所有非 Bash 工具使用
├── schemas.py         # MCP 工具 schema 与描述
├── tools/
│   ├── read.py write.py edit.py multi_edit.py multi_read.py file_stat.py bash.py glob.py grep.py feedback.py
│   ├── jobs.py job_kill.py job_archive.py job_script.py   # 面板工具（v0.3.0）
│   └── remote_info.py download.py upload.py
└── jobs/              # 面板子系统（v0.3.0）
    ├── __init__.py
    ├── sid.py         # derive_sid()，通过 PPID + 父进程启动时间（psutil）派生
    ├── paths.py       # 面板文件的本地 + 远端路径计算
    ├── init.py        # 启动时 mkdir + touch next_id + .id_lock
    ├── meta.py        # 读写 <id>-meta.json，fcntl flock id 分配，state 缓存写回
    ├── state.py       # 批量 kill -0 + date exec；4 态状态机推导
    ├── scripts.py     # status.sh 本地写 + SFTP 上传 + 首次运行 + 超时
...
- Background bash 日志默认位于 `~/.cache/remote-mcp-<sid>-<id>.log`，不在服务器退出时自动清理（故意——保留以供事后分析）。
- Background bash PID 重用是已知的低概率风险——agent 在发送 kill 信号前应 `kill -0 <pid>`。面板工具通过不重复观察终态任务来降低此风险。
- 面板状态不跨 Claude Code 重启保留（新 CC 进程 → 新 PPID → 新 `sid`）。任务仍在远端运行。恢复方式：`Bash("ls ~/.cache/remote-mcp-*-pid")`。
- 旧 session 的面板元数据（`~/.local/share/remote-mcp/jobpane/<old_sid>/`）不自动清理。可手动 `rm -rf`，或等待未来的 `JobsAdminCleanup` 工具（v0.3.0 未实现）。
