...
**Shell operations**
- For multi-step operations, prefer combining commands: `cmd1 && cmd2 && cmd3` in one Bash call. For more complex logic, write a script (Write to upload → Bash to execute).
...
- Tool call results carry a `[host=X cwd=Y]` prefix. Pay attention to which host you're operating on.
